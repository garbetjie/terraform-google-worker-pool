...
module worker {
  ...
  env = {}
  expose_ports = [{ port = 80, container_port = 8080, host = "127.0.0.1", protocol = "tcp" }]
...
| available_mounts                 | Mounts available to be used in timers and workers.                                                                                                                                    | list(object({ name = string, type = optional(string), src = string, target = string, readonly = optional(bool) }))      | `[]`            | No       |
| available_mounts.*.name          | Name of the mount. This is within `var.mounts` and `var.timers*.mounts` to reference the mount.                                                                                       | string                                                                                                                  |                 | Yes      |
| available_mounts.*.type          | Type of the mount. Available types can be found at https://docs.docker.com/storage. Currently only `volume` and `bind` mounts are supported.                                          | optional(string)                                                                                                        | `"volume"`      | No       |
| available_mounts.*.src           | Path on the host system to be mounted into the container.                                                                                                                             | string                                                                                                                  |                 | Yes      |
| available_mounts.*.target        | Target path in the container in which to place the mount.                                                                                                                             | string                                                                                                                  |                 | Yes      |
| available_mounts.*.readonly      | Makes the target path read only, and prevents writing to it.                                                                                                                          | optional(bool)                                                                                                          | `false`         | No       |
...
| expose_ports                     | Container ports to expose on the host. Passed directly to the -p flag. Should not be used if `var.workers_per_instance` > 1.                                                          | list(object({ port = number, protocol = optional(string), container_port = optional(number), host = optional(string)})) | `[]`            | No       |
...
| mounts                           | Names of mounts to mount into workers.                                                                                                                                                | set(string)                                                                                                             | `[]`            | No       |
...
| timers                           | Scheduled [timers](https://www.freedesktop.org/software/systemd/man/systemd.timer.html) to create.                                                                                    | list(object({ name = string, schedule = string, command = optional(list(string)) }))                                    | `[]`            | No       |
...
| timers.*.mounts                  | Names of mounts to mount into the timer. By default, uses the value from `var.mounts`. Set to `[]` to explicitly disable all mounts.                                                  | optional(set(string))                                                                                                   | `null`          | No       |
| timers.*.user                    | Optional user to run the timer as. Passed to the `-u` flag when running.                                                                                                              | string                                                                                                                  | `null`          | No       |
...
* **2.0.0**
  * Update how mounts are specified - all mounts are now specified in `var.available_mounts` with a name, and only the name
    is referenced in timers and workers.
    
  * Default values for `var.mounts` and `var.timers.*.mounts` are all names specified in `var.available_mounts`. This can
    be overridden by explicitly specifying `[]` for each. 
